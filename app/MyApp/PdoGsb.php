 ...
 */
	...
	/**fonction qui retourne les visiteurs */

	public function getLesVisiteurs($idVisiteur){
...
public function getLesInfosParType($leType){
	$req = "SELECT idVisiteur AS LeVisiteur, mois AS LeMois, (quantite* montant) AS LeMontant FROM lignefraisforfait INNER JOIN fraisforfait ON lignefraisforfait.idFraisForfait = fraisforfait.id WHERE fraisforfait.id = '" . $leType ."'";
	$res = $this->monPdo->query($req);
	$lesLignes = $res->fetchAll();
	return $lesLignes; 
}
...
/* tri les dates par ordre croissant */
...
public function getAfficher(){
	$req = "SELECT montant*quantite as total, id, idvisiteur, mois
	from fraisforfait
	inner join lignefraisforfait 
	on fraisforfait.id = lignefraisforfait.idFraisForfait
	where idVisiteur = 'a17' and mois = '201901' and id = 'ETP'
	";
	$res = $this->monPdo->query($req);
	$lesLignes = $res->fetchAll();
	return $lesLignes;
